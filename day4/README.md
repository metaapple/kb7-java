...
🎯 핵심 개념
개념	설명	예시
멤버변수	클래스의 속성, 전역변수	String size;, int price;
멤버메서드	클래스의 동작, 함수	powerOn(), powerOff()
자동 초기화	클래스 변수는 컴파일시 자동 초기화	String → null, int → 0
지역변수	메서드 내부 변수, 명시적 초기화 필요	int bonus = 10000;
메서드 오버로딩	같은 이름, 다른 입력값으로 정의 가능	getPrice(), getPrice(int sale)
